...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try { localStorage.setItem('megaden_has_used_app', 'true') } catch(e) {}
  ...
  return (
    <main style={{ background:'var(--navy)', minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'1.5rem', position:'relative', overflow:'hidden' }}>
      ...
      <div className="auth-card" style={{
        ...
        <h1 style={{ fontFamily:'Outfit, sans-serif', fontSize:'1.6rem', fontWeight:800, color:'var(--text)', marginBottom:'0.3rem', letterSpacing:'-0.5px' }}>Welcome Back</h1>
        <p style={{ color:'var(--muted)', fontSize:'0.85rem', marginBottom:'1.6rem', fontFamily:'Inter, sans-serif' }}>Log in to your MegaDen Exchange account</p>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'0.85rem', marginBottom:'1.4rem' }}>
          ...
          <span style={{ fontSize:'0.72rem', color:'var(--muted)', fontFamily:'Inter, sans-serif', whiteSpace:'nowrap' }}>or continue with email</span>
          <div style={{ flex:1, height:'1px', background:'var(--border)' }} />
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'0.9rem' }}>
          ...
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.4rem' }}>
              <label style={{ fontSize:'0.78rem', color:'var(--muted)', fontFamily:'Inter, sans-serif', fontWeight:500 }}>Password</label>
              <Link href="/forgot-password" style={{ fontSize:'0.76rem', color:'var(--purple2)', textDecoration:'none', fontFamily:'Inter, sans-serif', fontWeight:500 }}>Forgot password?</Link>
            </div>
            <div style={{ position:'relative' }}>
              <span className="input-icon"><LockIcon /></span>
              <input name="password" type={showPass ? 'text' : 'password'} placeholder="Your password" required value={form.password} onChange={handleChange} className="auth-input" style={{ paddingRight:'2.8rem' }} />
          ...
          <button type="submit" disabled={loading} className="submit-btn" style={{ marginTop:'0.3rem' }}>
            {loading
              ? <span style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'0.5rem' }}><Spinner /> Logging in…</span>
              : 'Log In →'
            }
          </button>
        </form>
...
function Spinner() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" style={{ animation:'spin 0.8s linear infinite', flexShrink:0 }}><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
}
