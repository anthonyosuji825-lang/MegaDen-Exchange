...
export default function SignUp() {
  ...
  return (
    <main style={{ background: 'var(--navy)', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* ── LOGO ── */}
      <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', textDecoration: 'none', marginBottom: '1.8rem', position: 'relative', zIndex: 1 }}>
        <LogoMark size={36} />
      ...
      <div className="auth-card" style={{
        ...
        <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1.65rem', fontWeight: 800, color: 'var(--text)', marginBottom: '0.35rem', letterSpacing: '-0.5px' }}>Create Account</h1>
        <p style={{ color: 'var(--muted)', fontSize: '0.88rem', marginBottom: '2rem', fontFamily: 'Inter, sans-serif' }}>Join MegaDen Exchange and start buying today</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            <label style={{ fontSize: '0.8rem', color: 'var(--muted)', display: 'block', marginBottom: '0.45rem', fontFamily: 'Inter, sans-serif', fontWeight: 500 }}>Full Name</label>
          ...
          <div>
            <label style={{ fontSize: '0.8rem', color: 'var(--muted)', display: 'block', marginBottom: '0.45rem', fontFamily: 'Inter, sans-serif', fontWeight: 500 }}>Password</label>
            <input name="password" type="password" placeholder="Min. 6 characters" required value={form.password} onChange={handleChange} className="auth-input" />
          </div>
          <div>
            <label style={{ fontSize: '0.8rem', color: 'var(--muted)', display: 'block', marginBottom: '0.45rem', fontFamily: 'Inter, sans-serif', fontWeight: 500 }}>Confirm Password</label>
            <input name="confirm_password" type="password" placeholder="Repeat your password" required value={form.confirm_password} onChange={handleChange} className="auth-input" />
          </div>
        ...
        {/* Trust badges */}
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginTop: '1.5rem', paddingTop: '1.2rem', borderTop: '1px solid var(--border)' }}>
          {['🔒 Secure', '⚡ Instant', '24/7 Support'].map(b => (
            <span key={b} style={{ fontSize: '0.72rem', color: 'var(--muted)', fontFamily: 'Inter, sans-serif' }}>{b}</span>
          ))}
        </div>

        <p style={{ textAlign: 'center', marginTop: '1.2rem', fontSize: '0.85rem', color: 'var(--muted)', fontFamily: 'Inter, sans-serif' }}>
