...
export default function VPN() {
  ...
  const handleOrder = async () => {
    ...
    await supabase.from('orders').insert({
      user_id: user.id,
      product_type: 'vpn',
      product_name: `${selectedProvider.name} - ${selectedPlan.name}`,
      amount: price,
      status: 'pending',
      details: { provider: selectedProvider.name, plan: selectedPlan.name, delivery: selectedPlan.delivery }
    })
    ...
    await supabase.from('transactions').insert({
      ...
      type: 'debit',
      amount: price,
      description: `${selectedProvider.name} VPN - ${selectedPlan.name}`,
      reference: `VPN-${Date.now()}`
    })
      ...
      <div style={{ padding: '1.2rem 1.4rem' }}>
        {success ? (
          <div style={{ textAlign: 'center', padding: '3rem 1rem', animation: 'fadeSlideIn 0.4s ease' }}>
            ...
            <div style={{ color: 'var(--muted)', fontSize: '0.85rem', marginBottom: '1rem' }}>Your VPN activation key will be delivered to your email shortly.</div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.8rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '0.8rem 1.4rem', margin: '0 0 1.5rem' }}>
            ...
            <br />
            <button onClick={() => { setSuccess(false); setSelectedProvider(null); setSelectedPlan(null) }} className="buy-btn"
              style={{ padding: '0.75rem 1.8rem', background: 'var(--purple)', color: '#fff', border: 'none', borderRadius: '12px', fontFamily: 'Outfit, sans-serif', fontSize: '0.9rem', fontWeight: 700 }}>
              Buy Another
            </button>
          </div>
        ) : (
                      ...
                      <button key={plan.id} className="plan-card"
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                          ...
                          <div style={{ textAlign: 'left' }}>
                            ...
                            <div style={{ fontSize: '0.7rem', color: 'var(--muted)', marginTop: '0.1rem' }}>Delivered in {plan.delivery} · Save {discount}%</div>
                          </div>
                        </div>
              ...
              <div style={{ animation: 'scaleIn 0.3s ease both' }}>
                <div style={{ background: 'var(--card2)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.1rem', marginBottom: '1rem' }}>
                  ...
                  {[
                    ...
                    { label: 'Delivery', value: selectedPlan.delivery },
